...
def main():
    ...
    val_data = load_dataset('lm1b', split='test[:10000]', trust_remote_code=True)
    ...
    val_sentences = val_sentences.filter(lambda ex: len(ex["tokens"]) <= 60)
    ...
    train_sentence_ids = [numericalize_sentence(s['tokens'], tok2id, tok2id['<unk>']) for s in train_sentences]
    ...
    val_sentence_ids = [numericalize_sentence(s['tokens'], tok2id, tok2id['<unk>']) for s in val_sentences]
    val_ds = FullSentenceDataset(val_sentence_ids, tok2id['<pad>'])
    val_loader = torch.utils.data.DataLoader(val_ds, batch_size=16, shuffle=False, num_workers=2, pin_memory=True, collate_fn=collate_full_sentences)
    ...
    file_path = 'lstm_model_30_epochs_input50_numlayers2_hidden512_lr0.0001_batchsize64_L2_1e-6.pth'
